...
export default class Dial extends Interface {
  ...
  constructor() {
    ...
    let defaults = {
      'size': [80,80],
      'interaction': 'vertical', // radial, vertical, horizontal
  ...
  move() {
    if (this.clicked) {
      ...
      let angle = this.position.value*Math.PI*2 - Math.PI*0.5;

      if (angle < 0 ) { angle += (Math.PI*2); }

      if (this.previousAngle !== false || this.mode === 'relative') {
        if (Math.abs(this.previousAngle - angle) > 2) {
