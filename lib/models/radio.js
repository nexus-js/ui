...
export default class Radio {
    ...
    constructor(length = 3, ...onVals) {
        ...
        //In the example below, a 3-button radio is created, index 0 is switched on, index 1 is switched on then then attempted again, and the final argument is ignored because the index value does not exist.
        //Example:
        //`  radio = new Radio(3, 0, 1, 1, 3);
        //…  [1,1,0]
        /*
                if (!isNaN(length) && (function(x) { return (x | 0) === x; })(parseFloat(length))) { //provided length must be some form of an integer value, or is set to 1
                    length = 1;
                }
        */
    ...
    flip(...values) {
        ...
        if (values.length > 0) {
            values.forEach(function(v) {
                if (v > a.length - 1) {
                    console.error('Warning: AnonRadio[' + v + '] does not exist');
                } else {
                    a[v] = (a[v] ? 0 : 1);
                }
            });
    ...
    on(...values) {
        ...
        if (values.length > 0) {
            values.forEach(function(v) {
                if (v > a.length - 1) {
                    console.error('Warning: AnonRadio[' + v + '] exceeds size of object');
                } else {
                    if (a[v] === 1) { console.error('Warning: AnonRadio[' + v + '] was already on.'); }
                    a[v] = 1;
                }
            });
        } else {
