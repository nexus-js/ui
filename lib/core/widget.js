...
*/
  ...
  // TODO: (EM) I think widget should be passed the canvas an argument. and then 
  // we don't need a canvasID field
  /**  @property {string} canvasID ID attribute of the interface's HTML5 canvas */
  this.canvasID = target;

  // TODO: (EM) I think for some purposes a widgetID might be usefull 
  // (instead of canvasID). See later. 
  // TODO: (EM) I think oscPath should be independent of the canvasID or widgetID,
  // in case we want 2 widgets to have the same oscPath. BTW: this is a mistake
  // that exists in Lemur also. 
  ...
  // TODO: (EM) I think canvas creation should be moved to the manager.transform()
  // and the actual canvas should be passed as argument to the widget's constructor.
  ...
  // TODO: (EM) again canvas should be an argument, to avoid DOM queris
  this.canvas = document.getElementById(target);
  /**  @property {HTML5 drawing context} context The canvas's drawing context */
  this.context = this.canvas.getContext("2d");

  // TODO: (EM) This updates the widget state from the canvas.style. Nees to put this in a
  // updateWidgetFromCanvas() 
  this.checkPercentage();

  // FIXME: (EM) don't append 'nx' if 'nx' already exists on canvas.
  this.canvas.className = this.canvas.className ? this.canvas.className += " nx" : "nx"

  const computedHeightString = window.getComputedStyle(document.getElementById(target), null).getPropertyValue("height").replace("px","");
  const computedWidthString = window.getComputedStyle(document.getElementById(target), null).getPropertyValue("width").replace("px","");
  this.canvas.height = computedHeightString;
  this.canvas.width = computedWidthString;
  // TODO: this updates the widget's state from the canvas, need to put in an updateWidgetFromCanvas(),
  // maybe include the computedStyle as well.
  /**  @property {integer} height The widget canvas's computed height in pixels */
  this.height = parseInt(computedHeightString);
  /**  @property {integer} width The widget canvas's computed width in pixels */
  this.width = parseInt(computedWidthString);
  if (!this.defaultSize) {
  ...
  // TODO: (EM) This sould be put in an updateWidgetFromCanvas()
  ...
  // TODO: (EM) I don't understand this 2 factor scaling. When should this initialization happen?
  // If the canvas.width is updated externally, what should happen?
  // If the canvas.style.width is updated externally, what should happen?
  this.canvas.style.width = this.canvas.width/2+"px";
  this.canvas.style.height = this.canvas.height/2+"px";
  this.context.scale(2,2);

  // TODO: all size related initialization should be moved to a setSize() method.
  // See comments in widget.resize().
  ...
  /**  @property {object} center The center of the widget's canvas (not including the label). A 100x100 widget would have a center at 50x50. (Has properties 'x' and 'y', both in pixels) */
  this.center = {
    x: this.GUI.w/2,
    y: this.GUI.h/2
  };
  ...
  // TODO: (EM) this is updated from widget.lineWidth in the draw() method of each widget,
  // it should work, this is something I would put in widget.init() or widget.updateCanvas().
  // In general I think there should be an updateCanvasFromWidget() when the widget state changes, 
  // and an updateWidgetFromCanvas() when the canvas state changes. Currently the state is spread
  // over both widget and canvas. Actually it would be best if the state would be only on widget,
  // and stuff like context.lineWidth would be set everytime when needed, like in the draw() method.
  // So actually this is a good example which if followed through means that there is no need for updateCanvasFromWidget().
  // Another alternative is to have state on the widget and use an updateCanvasFromWidget(), and 
  // not allow (by documentation) any changes to be made to the canvas directly, but onloy through the widget.
  ...
  // TODO: (EM) there is an exception to this filed's definition. With the matrix widget, the 
  // matrix data sits on the 'matrix' field and not the 'val' field. And the 'val' is 
  // used to transmit changes. It would be nice to have a field which is the core data of 
  // all widgets.
    /**
...
widget.prototype.resize = function(w,h) {
  ...
  // TODO: I would extract all this method to a "private" setSize()  (not on widget) excluding the init() and draw(),
  // and use this  setSize() method  in the constructor. That avoids duplicate code.
  ...
  // FIXME: Need to call makeRoomForLabel() firstm in order to update this.GUI 
  // with the new w,h
  this.center = {
    x: this.GUI.w/2,
    y: this.GUI.h/2
