...
export default class Interface extends EventEmitter {
  ...
  attachListeners() {
    ...
    // Setup interaction
    if (touch.exists) {
      this.element.addEventListener('touchstart', evt => this.preTouch(evt));
      this.element.addEventListener('touchmove', evt => this.preTouchMove(evt));
      this.element.addEventListener('touchend', evt => this.preTouchRelease(evt));
    }
