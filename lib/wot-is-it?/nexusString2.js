	...
	this.draw = function() {
		...
		with (self.context) {
			...
			/*	if (self.abovestring[i] != (self.clickPos.y<self.strings[i].y1) ) {
				...
				if (Math.abs(self.clickPos.y-self.strings[i].y1)<10) {
						self.strings[i].on = true;	
				}

				if (Math.abs(self.clickPos.y-self.strings[i].y1)<stringdiv/2 && self.strings[i].on) {
				...
				//	}
					...
					if (self.strings[i].on) {
						...
						self.nxTransmit([i,self.clickPos.x]);
					}
				}
			}
		}
