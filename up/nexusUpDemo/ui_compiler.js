...
var thisfolder = patch.filepath;
thisfolder = thisfolder.replace(patch.name+".maxpat", "");
...
function findUIObjects()
{
	...
	var current_object = patch.firstobject;
...
{
	var receiver = patch.newdefault(50, 10, "udpreceive", port);
	var printer = patch.newdefault(50, 40, "print", "incoming");
	patch.connect(receiver, 0, printer, 0);
}
...
function getQRCode()
{
	...
	address = "http://qrfree.kaywa.com/?l=1&s=8&d=" + address +patch.name + ".html alt=QRCode/";
	outlet(0, "qr", address);
...
//	outlet(0, oscVal);
	...
	var elemToSet = patch.getnamed(oscName);
...
<body>                                                                                                \
	...
	<div style="position:relative;margin:0;padding:2px 10px;background-color:#0cf">     \
		<div style="float:left;font-size:14pt;opacity:0.7;">'+patch.name+'.maxpat</div>     \
	...
	f = new File(thisfolder+patch.name+".html", "readwrite");
	f.writestring(html);
	f.close;
}
...
function addButton(val)
{
	for(i=0;i<val;i++)
	{
		uiObjects[i] = patch.newdefault(200+(i*50), 50, "button", i+1);
	}
}
